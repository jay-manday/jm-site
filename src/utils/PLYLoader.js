...
class PLYLoader {
  ...
  isASCII(data) {
    let header = this.parseHeader(this.bin2str(data))
  ...
  parseHeader(data) {
    let patternHeader = /ply([\s\S]*)end_header\s/
    let headerText = ''
    let headerLength = 0
    ...
    const make_ply_element_property = (propertValues, propertyNameMapping) =>{
  ...
  parseASCIIElement(properties, line) {
    ...
    for (let i = 0; i < properties.length; i ++) {
      if (properties[i].type === 'list') {
        let list = []
        let n = this.parseASCIINumber(values.shift(), properties[ i ].countType)
        for (var j = 0; j < n; j ++) {
  ...
  parseASCII(data) {
    ...
    for (let i = 0; i < lines.length; i ++ ) {
      ...
      let element = this.parseASCIIElement(header.elements[currentElement].properties, line)
  ...
  handleElement(geometry, elementName, element) {
    if (elementName === 'vertex') {
      ...
      let vertex_indices = element.vertex_indices
